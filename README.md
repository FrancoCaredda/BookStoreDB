...
|BK-007|Verify that there are no order items with the quantity less than or equal to zero|Active connection to the database + imported data|1. Select order items with the quantity less than or equal to zero; 2. Observe the results|No rows are returned|High|
